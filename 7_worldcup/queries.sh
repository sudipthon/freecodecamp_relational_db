...
echo "$($PSQL "SELECT DISTINCT name FROM teams JOIN games ON teams.team_id = games.winner_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT DISTINCT name  FROM teams JOIN games ON teams.team_id = games.opponent_id WHERE year = 2014 AND round = 'Eighth-Final'; ")"
